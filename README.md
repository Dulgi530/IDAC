...
| 보고서 화면용 스냅샷 | `GET /api/snapshot?period=YYYY-MM` · `GET /api/snapshots` |
...
| 경로 | 설명 | 접근 |
| --- | --- | --- |
| `/` | **평가 보고서 단일 페이지** — 요약 지표·순위표·행 펼침 상세 | 퍼블릭 |
| `/reports/{YYYY-MM}` | 리포트 전문 (문서형 레이아웃) | 퍼블릭 |
| `/methodology` | 평가 기준과 배점 공개 | 퍼블릭 |
| `/admin` | 리포트 생성 | 관리자 도메인 전용 (그 외에서는 404) |

`/` 는 한 화면에서 보고서를 끝까지 읽도록 구성했다.

```
헤더        기간 · 개요 · 발행된 월 칩 · Excel/PDF/인쇄
방법론      A → B → C∼F 단계와 GTF 변동성 산출 기준 (접이식)
데이터 출처  집계에 사용한 API·자료 카드
요약 지표    평가 코인 수 · 총 시가총액 · 평균 1년 변동성 · 국내 4사 이상 상장
유의사항     데이터 신뢰도 경고 (fixture 모드, 미검증 시세 등)
순위표      30종 — 행을 펼치면 재단·활용·SNS·거래소별 수수료·총평
꼬리말      집계 시점 · 국내 KRW 마켓 커버리지 · 후보군 규모
```

월 전환 칩은 `/api/snapshot` 을 호출해 화면 안에서 보고서를 갈아 끼운다.
요청한 달이 아직 발행 전이면 가장 최근 발행분을 대신 보여주고 그 사실을
헤더에 표시한다. 최초 렌더링은 서버에서 끝내므로 빈 화면이 스치지 않는다.

사이트 헤더가 붙는 부속 화면(`/reports/*`, `/methodology`, `/admin`)은
`src/app/(site)` 라우트 그룹의 레이아웃이 감싸고, `/` 는 전체 화면을 그대로 쓴다.
...
변동성은 1년 연환산 값 외에 **GTF 90D · 180D** 구간 값을 함께 낸다. 엑셀 산식과
동일하게 일별 로그수익률 `LN(Pₜ / Pₜ₋₁)` 에 표본 표준편차(`STDEV.S`)를 적용하고,
24/7 거래되는 디지털자산 기준 연환산 계수 `√365` 를 곱한다. 구간 값은 일봉
시계열이 있어야 산출되므로 **fixture 모드에서는 `—` 로 표기**되고, 그 사실이
리포트 유의사항에 남는다.
